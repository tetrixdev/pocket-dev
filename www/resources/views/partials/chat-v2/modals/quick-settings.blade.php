                ...
                <div class="space-y-2">
                    <template x-for="(p, key) in providers" :key="key">
                        <label class="flex items-center text-gray-300 cursor-pointer" :class="{'opacity-50': !p.available}">
                            <input type="radio" x-model="provider" :value="key" @change="updateModels()" :disabled="!p.available" class="mr-2">
                ...
                <div class="space-y-2 max-h-48 overflow-y-auto">
                    <template x-for="(info, modelId) in availableModels" :key="modelId">
                        <label class="flex items-center text-gray-300 cursor-pointer">
                            <input type="radio" x-model="model" :value="modelId" class="mr-2">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <template x-for="(mode, index) in thinkingModes" :key="index">
                        <label class="flex items-center text-gray-300 cursor-pointer">
                            <input type="radio" x-model="thinkingLevel" :value="index" @change="localStorage.setItem('thinkingLevel', thinkingLevel)" class="mr-2">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <template x-for="(mode, index) in responseModes" :key="index">
                        <label class="flex items-center text-gray-300 cursor-pointer">
                            <input type="radio" x-model="responseLevel" :value="index" @change="localStorage.setItem('responseLevel', responseLevel)" class="mr-2">
                            <span x-text="mode.icon" class="mr-1"></span>
                            <span x-text="mode.name"></span>

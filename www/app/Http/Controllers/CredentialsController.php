    ...
    public function processSetup(Request $request)
    {
        ...
        try {
            ...
            if ($validated['provider'] === 'claude_code') {
                ...
                $exitCode = 0;
                $queueContainer = escapeshellarg(config('pocketdev.project_name', 'pocket-dev') . '-queue');
                exec("docker exec {$queueContainer} chmod 660 /home/appuser/.claude.json /home/appuser/.claude.json.backup 2>/dev/null", $output, $exitCode);
                if ($exitCode !== 0) {
                    Log::warning('Claude config chmod failed', ['files' => '.claude.json', 'exit_code' => $exitCode]);
                }
                exec("docker exec {$queueContainer} chmod 664 /home/appuser/.claude/settings.json 2>/dev/null", $output, $exitCode);
                if ($exitCode !== 0) {

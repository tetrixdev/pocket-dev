...
if [ "$SKIP_DOMAIN" = true ]; then
    ...
    # =============================================================================
    # When using Tailscale restriction, users need split DNS so their domain
    # resolves to the Tailscale IP (not public IP) when connected to Tailscale.
    # This sets up CoreDNS to answer DNS queries for the domain.
    COREDNS_READY=false
    if [ "$RESTRICTION" = "tailscale" ]; then
        echo ""
        log_info "Setting up CoreDNS for Tailscale split DNS..."

        # Check if Tailscale CLI is available
        if ! command -v tailscale &> /dev/null; then
            log_warn "Tailscale CLI not found. Skipping CoreDNS setup."
            log_warn "Install Tailscale CLI or configure split DNS manually."
        else
            # Get Tailscale IP dynamically
            TAILSCALE_IP=$(tailscale ip -4 2>/dev/null || echo "")
            if [ -z "$TAILSCALE_IP" ]; then
                log_warn "Tailscale not connected. Skipping CoreDNS setup."
                log_warn "Run 'tailscale up' and re-run installer to enable split DNS."
            else
                ...
                if ss -tuln 2>/dev/null | grep -q "$TAILSCALE_IP:53"; then
                    ...
                    # Generate Corefile with domain and Tailscale IP
                    # Uses template plugin for wildcard subdomain support
                    cat > "$COREDNS_DIR/Corefile" << EOF
# CoreDNS configuration for PocketDev split DNS
# Domain: $DOMAIN -> $TAILSCALE_IP
# Generated by PocketDev installer

# Handle base domain and all subdomains
$DOMAIN {
    # Base domain - exact match
    hosts {
        $TAILSCALE_IP $DOMAIN
        fallthrough
    }
    # Wildcard subdomains - matches *.domain via template
    template IN A {
        match ^.*\.$DOMAIN\.$
...
EOF
                    ...
                    # Stop existing CoreDNS if running
                    docker rm -f coredns 2>/dev/null || true

                    # Run CoreDNS container (bind to Tailscale IP only to avoid conflicts)
                    if docker run -d --name coredns --restart=unless-stopped \
                        ...
                        coredns/coredns -conf /Corefile; then
                        log_info "CoreDNS started successfully"
                        COREDNS_READY=true
                    else
                        log_warn "Failed to start CoreDNS container"
                    fi
                fi
            fi
        fi
    ...
    case "$RESTRICTION" in
        tailscale)
            ...
            if [ "${COREDNS_READY:-false}" = true ]; then
                echo "  ✓ CoreDNS running for split DNS"
            fi
            ;;
        whitelist)

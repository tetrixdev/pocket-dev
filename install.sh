...
# Port binding is NOT in compose.yml - it's added here based on mode
if [ "$PROXY_AVAILABLE" = true ] && [ "$SKIP_DOMAIN" = false ]; then
    # Production mode with proxy-nginx: no port binding, connect via main-network
...
EOF
    ...
    # Local/standalone mode: bind nginx to configured port
    cat > compose.override.yml << EOF
# Override for local/standalone mode
# Generated by PocketDev installer
...
services:
  pocket-dev-nginx:
    ports:
      - "\${PD_NGINX_PORT:-80}:80"
EOF
    log_info "Created compose.override.yml for local mode (port $ARG_PORT)"
fi
...
if [ "$SKIP_DOMAIN" = true ]; then
    ...
    # =============================================================================
    # Split DNS Zone Registration (Tailscale mode only)
    # =============================================================================
    # When using Tailscale restriction, register this domain in CoreDNS.
    # CoreDNS should already be running (set up by VPS Setup with --tailscale).
    COREDNS_READY=false
    if [ "$RESTRICTION" = "tailscale" ]; then
        echo ""
        log_info "Configuring split DNS for Tailscale..."

        # Get Tailscale IP
        TAILSCALE_IP=$(tailscale ip -4 2>/dev/null || echo "")
        if [ -z "$TAILSCALE_IP" ]; then
            log_warn "Tailscale not connected. Skipping split DNS setup."
        else
            log_info "Detected Tailscale IP: $TAILSCALE_IP"

            # Check if CoreDNS is running (should be set up by VPS Setup)
            if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^coredns$'; then
                log_warn "CoreDNS is not running."
                log_warn "Split DNS requires CoreDNS to be set up by VPS Setup."
                echo ""
                echo "To fix this, run VPS Setup with --tailscale flag:"
                echo "  curl -fsSL https://raw.githubusercontent.com/tetrixdev/vps-setup/main/setup.sh | sudo bash -s -- --tailscale"
                echo ""
            else
                # CoreDNS is running - register our domain
                ZONE_DIR="/etc/coredns/zones.d"
                ZONE_FILE="$ZONE_DIR/pocketdev-${ARG_NAME}.conf"

                # Check zone directory exists
                if [ ! -d "$ZONE_DIR" ]; then
                    log_warn "Zone directory $ZONE_DIR does not exist."
                    log_warn "VPS Setup may need to be re-run with --tailscale."
                else
                    # Create zone file for this PocketDev instance
                    cat > "$ZONE_FILE" << EOF
# PocketDev zone file
# Domain: $DOMAIN
# Instance: ${ARG_NAME}
...
if [ "$SKIP_DOMAIN" = true ]; then
    ...
    case "$RESTRICTION" in
        tailscale)
            ...
            if [ "${COREDNS_READY:-false}" = true ]; then
                echo "  ✓ Domain registered in CoreDNS"
            fi
            ;;
        whitelist)
    ...
    # Show Tailscale split DNS instructions if applicable
    # Note: TAILSCALE_IP was already set during CoreDNS setup
    if [ "$RESTRICTION" = "tailscale" ] && [ "$COREDNS_READY" = true ]; then
        ...
        echo ""
        echo "If it shows a different IP, try: ipconfig /flushdns (Windows) or"
        echo "sudo dscacheutil -flushcache (Mac) and check again."
        echo ""

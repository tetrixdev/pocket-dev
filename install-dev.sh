...
if [ "$SKIP_DOMAIN" = true ]; then
    ...
    if [ "$RESTRICTION" = "tailscale" ]; then
        ...
        if [ -z "$TAILSCALE_IP" ]; then
            log_warn "Could not detect Tailscale IP. Skipping CoreDNS setup."
            log_warn "Split DNS will need to be configured manually."
        else
...
if [ "$SKIP_DOMAIN" = true ]; then
    ...
    if [ "$RESTRICTION" = "tailscale" ] && [ "${COREDNS_READY:-false}" = true ]; then
